...
**Find the browse binary (`$B`):**

This skill uses a headless browser automation tool. Locate it:
...
If `NEEDS_SETUP`: tell the user the browse tool is not available and ask how they'd like to proceed (they may need to install it separately or provide screenshots manually).
...
$B url
```
If URL contains `/login`, `/signin`, `/auth`, or `/sso`: the site requires authentication. AskUserQuestion: "This site requires authentication. How would you like to proceed? (provide credentials, import cookies, or skip auth-required pages)"
...
11. **Show screenshots to the user.** After every `$B screenshot`, `$B snapshot -a -o`, or `$B responsive` command, use the Read tool on the output file(s) so the user can see them inline. For `responsive` (3 files), Read all three.
...
If the fix involved JS behavior: write a regression test encoding the exact bug condition, run it, commit if it passes or defer if it fails. Commit format: `test(design): regression test for FINDING-NNN`.
